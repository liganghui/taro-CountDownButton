...
export default class DownButton extends Component {
  ...
  static defaultProps = {
    ...
    onClick: () => {console.log('当显示此信息时 , 说明倒计时按钮没传入点击事件')}, //按下按钮的事件,但是触发倒数(startCountDown)需要你自己来调用
    changeWithCount: (count) => {return count+'秒'}, //读秒变化的函数,该函数带有一个参数count,表示当前的剩余事件
    end: () => {}, //读秒完毕后的回调,读秒结束触发
    frameStyle: {}, //初始化的位置大小
